defmodule ExOpenSea.Assets.ListTest do
  use ExUnit.Case, async: false
  use ExVCR.Mock, adapter: ExVCR.Adapter.Hackney
  import Mock
  doctest ExOpenSea.Assets.List
  ...
  test ".get/1" do
    use_cassette "assets/list/get_ok" do
      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key)
  ...
  test ".get/2 can filter by collection" do
    use_cassette "assets/list/get_filter_collection_ok" do
      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{collection: "doodles-official"})
  ...
  test ".get/2 can filter by collection_slug" do
    use_cassette "assets/list/get_filter_collection_slug_ok" do
      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{collection_slug: "azuki"})
      assert length(cursor.assets) == 20
  ...
  test ".get/2 can filter by token_ids" do
    use_cassette "assets/list/get_filter_token_ids_ok" do
      token_ids = [1, 2]
      str_token_ids = token_ids |> Enum.map(&Integer.to_string/1)

      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{collection_slug: "azuki", token_ids: token_ids})
  ...
  test ".get/2 can filter by owner" do
    use_cassette "assets/list/get_filter_owner_ok" do
      owner = "0x5117fa741c86921b0910e889d4123ec111349fa3"

      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{collection_slug: "azuki", owner: owner})
  ...
  test ".get/2 can filter by order_direction" do
    use_cassette "assets/list/get_filter_order_direction_ok" do
      params = %{collection_slug: "ocm-dessert", token_ids: [1], include_orders: true, order_direction: "desc"}

      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, params)
      assert length(cursor.assets) == 1
      assert %ExOpenSea.Asset{} = asset = Enum.at(cursor.assets, 0)
  ...
  test ".get/2 can filter by asset_contract_address" do
    use_cassette "assets/list/get_filter_asset_contract_address_ok", match_requests_on: [:query] do
      assert {:ok, cursor_1} = ExOpenSea.Assets.List.get(@api_key, %{asset_contract_address: @azuki_contract_address, limit: 1})
      ...
      assert {:ok, cursor_2} = ExOpenSea.Assets.List.get(@api_key, %{asset_contract_address: @doodles_contract_address, limit: 1})
  ...
  test ".get/2 can filter by asset_contract_addresses" do
    use_cassette "assets/list/get_filter_asset_contract_addresses_ok" do
      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{asset_contract_addresses: [@azuki_contract_address, @doodles_contract_address], token_ids: [1, 1]})
      assert length(cursor.assets) == 2
      assert %ExOpenSea.Asset{} = asset_1 = Enum.at(cursor.assets, 0)
  ...
  test ".get/2 can filter by limit" do
    use_cassette "assets/list/get_filter_limit_ok" do
      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{collection_slug: "azuki", limit: 1})
      assert length(cursor.assets) == 1
      assert %ExOpenSea.Asset{} = asset = Enum.at(cursor.assets, 0)
  ...
  test ".get/2 can filter by cursor" do
    use_cassette "assets/list/get_filter_cursor_ok", match_requests_on: [:query] do
      assert {:ok, cursor_1} = ExOpenSea.Assets.List.get(@api_key, %{limit: 1})
      assert length(cursor_1.assets) == 1
      assert %ExOpenSea.Asset{} = asset_1 = Enum.at(cursor_1.assets, 0)
      assert asset_1.name != nil

      assert {:ok, page_2} = ExOpenSea.Assets.List.get(@api_key, %{cursor: cursor_1.next, limit: 1})
  ...
  test ".get/2 can filter by include_orders" do
    use_cassette "assets/list/get_filter_include_orders_ok" do
      assert {:ok, cursor} = ExOpenSea.Assets.List.get(@api_key, %{collection_slug: "azuki", token_ids: [1378], include_orders: true})
      assert %ExOpenSea.Asset{} = asset = Enum.at(cursor.assets, 0)
      assert length(asset.sell_orders) == 1
    end
  end
  ...
  test ".get/n bubbles error tuples" do
    with_mock HTTPoison, request: fn _url -> {:error, %HTTPoison.Error{reason: :timeout}} end do
      assert ExOpenSea.Assets.List.get(@api_key) == {:error, :timeout}
    end
  end
end
